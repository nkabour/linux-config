...
local function factory(args)
    ...
    pulse.cmd = args.cmd or "pacmd list-" .. pulse.devicetype .. "s | grep -e $(pactl info | grep -e 'ink' | cut -d' ' -f3) -e 'volume: front' -e 'muted' -e 'index' -e 'device.string' | tail -n 9 | head -n 5"
     --"s | sed -n -e '/*/,$!d' -e '/index/p' -e '/base volume/d' -e '/volume:/p' -e '/muted:/p' -e '/device\\.string/p' | head -n 4"
